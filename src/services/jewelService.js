...
export function initDatabase(){
  ...
  db.transaction(tx =>{
    tx.executeSql('CREATE TABLE IF NOT EXISTS Jewel' +
                  '(id INTEGER PRIMARY KEY AUTOINCREMENT, jewelType TEXT ,jewelId INTEGER, description TEXT, gold INTEGER, weight REAL, price INTEGER, note TEXT, date TEXT)')
...
export async function postJewel(type, jewelId, description, gold, weight, price, note, date){
  const db = SQLite.openDatabase('Jewellery.db')
  db.transaction(tx =>{
    tx.executeSql('INSERT INTO Jewel (jewelType, jewelId, description, gold, weight, price, note, date) VALUES (?,?,?,?,?,?,?,?)',              
      [type, jewelId, description, gold, weight, price, note, date],
